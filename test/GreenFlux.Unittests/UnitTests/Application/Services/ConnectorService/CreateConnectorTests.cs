...
{
    public class CreateConnectorTests
    {
        ...
        [Fact]
        public void GivenInvalidGroupId_WhenCreatingConnector_NotFoundExceptionShouldBeThrown()
        {
            Assert.Throws<NotFoundException>(() => _target.CreateConnector(Guid.NewGuid(), _chargeStationId, new SaveConnectorDto()));
        }

        [Fact]
        public void GivenInvalidChargeStationId_WhenCreatingConnector_NotFoundExceptionShouldBeThrown()
        {
            Assert.Throws<NotFoundException>(() => _target.CreateConnector(_groupId, Guid.NewGuid(), new SaveConnectorDto()));
        }


        [Fact]
        public void GivenValidIds_AndValues_WhenCreatingConnector_ConnectorShouldBeAddedToDomainModel()
        {
            //arrange
            ...
            //act
            _target.CreateConnector(_groupId, _chargeStationId, saveConnectorDto);

            //assert
            Assert.Equal(2, _chargeStation.ConnectorCapacities.Count());
        }

        [Fact]
        public void GivenValidIds_AndValues_WhenCreatingConnector_ChargeStationShouldBeSavedAfterConnectionIsAdded()
        {
            //arrange
            var saveConnectorDto = new SaveConnectorDto { MaxCurrentInAmps = 5 };
            _repository.Setup(m => m.SaveChargeStation(It.Is<ChargeStation>(c => c.ConnectorCapacities.Count() == 2))).Returns(true);

            //act
            _target.CreateConnector(_groupId, _chargeStationId, saveConnectorDto);

            //assert
            _repository.Verify(m => m.SaveChargeStation(It.Is<ChargeStation>(c => c.ConnectorCapacities.Count() == 2)), Times.Once);
        }

        [Fact]
        public void GivenValidIds_AndValues_WhenCreatingConnector_ResultShouldBeConnectorsMapperResult()
        {
            //arrange
            var saveConnectorDto = new SaveConnectorDto { MaxCurrentInAmps = 5};
            short expectedConnectorId = 2;
            var connectorDtoMapperResult = new ConnectorDto();
            _connectorDtoMapper.Setup(m => m.Map(_chargeStation, expectedConnectorId)).Returns(connectorDtoMapperResult);
            _repository.Setup(m => m.SaveChargeStation(_chargeStation)).Returns(true);

            //act
            var result = _target.CreateConnector(_groupId, _chargeStationId, saveConnectorDto);
